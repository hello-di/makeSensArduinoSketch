...
bool lib_aci_event_get(aci_state_t *aci_stat, hal_aci_evt_t *p_aci_evt_data)
{
  ...
  /*
  When there is still space in the Event queue, check if there are any event or command that require pocessing.
  When the event queue is full, do not process the nRF8001 transactions
  Since we de-queued an event from the event queue, the queue cannot be full
  */
  // if (true == m_aci_q_is_full(&aci_rx_q))
  // {	
	// return status;
  // }
  ...
  */
  if (0 != digitalRead(HAL_IO_RADIO_RDY))
  {
  ...
  /*
  Process the ACI Event that has been placed in the queue
  */
